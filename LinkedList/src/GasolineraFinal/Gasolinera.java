...
public Gasolinera() {
	this.arraySurtidores = new Surtidor[4];//IMPORTANTE solo HA DECLARADO QUE HAY posiciones quevalen null
	
	for (int i = 0; i < arraySurtidores.length; i++) {
		arraySurtidores[i]=new Surtidor();//secrean surtidores pero con sus campos a null,0,o false(Por defecto)
	}
...
public void metodo1(Coche coche) 
{
	...
	boolean vacio = false;
	for (int i = 0;  i < arraySurtidores.length &&  vacio ; i++) {
...
*/
		if (arraySurtidores[i].getColaCoches().isEmpty()) {//en el primero vacio
			System.out.println("I F");
			arraySurtidores[i].setCoche(coche);
...
public void metodo2() {
	...
	int numero;
	numero=in.nextInt();
	System.out.println("acaba de abandonar la cola el vehiculo "+arraySurtidores[numero].getColaCoches().pop());
}
...
public void metodo3() {
	...
	for (int i = 0; i < arraySurtidores.length; i++) {
		System.out.println(arraySurtidores[i].toString());
	}
...
public Surtidor buscarSurtidor() {
	...
	for (int i = 0; i < arraySurtidores.length; i++) {
		/*arraySurtidores[i].codigo=i;*/ //otra forma de meter el codigo
		   if (arraySurtidores[i].getColaCoches().size() < minimo) {
			minimo=arraySurtidores[i].getColaCoches().size();//numero de coches
			posicion=i;
		}
	...
	return arraySurtidores[posicion];
